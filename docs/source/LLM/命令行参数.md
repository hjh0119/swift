...
- `--loss_scale_config_path`: `use_loss_scale`为True时生效，表示配置Agent特殊字段和对应的loss scale的配置路径. 其中键对应特殊字段, 值表示特殊字段和后续字段的loss_scale。以`"Observation:": [2, 0]`为例，比如response为`xxxx Observation:error"`, 在计算loss时会将`Observation:`部分的字段loss扩大两倍，不计算`error`的部分。同时支持正则匹配式，键对应匹配模式，值表示匹配字段的loss_scale, 例如：`"r'<.*?>'":[1.0]`. 默认为DEFAULT, 使用 `/swift/llm/agent/loss_scale_config.json`
