...
- `loss_scale_config_path`: This takes effect when `use_loss_scale` is set to True, indicating the path to the configuration for Agent special fields and their corresponding loss scales. The keys correspond to special fields, and the values indicate the loss scale for the special field and subsequent fields. For example, with `"Observation:": [2, 0]`, if the response is `xxxx Observation:error`, during the calculation of loss, the part of the field `Observation:` will have its loss doubled, and the part of `error` will not be calculated. Supports regular expression matching, where the key corresponds to the matching pattern and the value represents the loss scale of the matched field, for example: `"r'<.*?>'":[1.0]`.The default is `DEFAULT`, using `/swift/llm/agent/loss_scale_config.json` as config.
