...
from ..vision_utils import load_audio, load_audio_vllm, video_get_metadata, video_to_ndarrays
    ...
    def replace_tag(self, media_type: Literal['image', 'video', 'audio'], index: int,
        ...
        if media_type == 'image':
            ...
            if self.mode != 'vllm':
                ...
                inputs.audios[index], _ = load_audio_vllm(inputs.audios[index], sr=sr, mono=True)
            return ['<|audio|>']
            ...
            if self.mode == 'vllm':
                ...
                inputs.videos[index] = video_data
            return ['<|video|>']
