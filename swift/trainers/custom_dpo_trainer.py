...
class DPOTrainer(PushToMsHubMixin, SwiftMixin, HFDPOTrainer):
    ...
    def __init__(self, *args, template: Template, sft_beta=0., test_oom_error=False, **kwargs):
        dpo_config: Optional[DPOConfig] = kwargs.get('args')
        is_vision = kwargs.pop('is_vision')
        self.streaming = kwargs.pop('streaming')

        SwiftMixin.__init__(self, *args, **kwargs)

        self.set_config(dpo_config)
        self.is_encoder_decoder = kwargs.get('model').config.is_encoder_decoder
        if kwargs.get('ref_model', None) is not None:
    ...
    def set_config(self, dpo_config: DPOConfig):
