...
class RolloutTrainerArgumentsMixin(VllmArguments):
    ...
    Args:
        ...
        max_staleness (Optional[int]): Maximum allowed policy version difference between rollout and training
            when `async_generate` is enabled. Samples generated by a policy version that differs from the current
            training version by more than this threshold will be discarded. When set to 1, behaves like traditional
            async_generate (1-step off-policy). When set to larger values, allows more off-policy samples for higher
            throughput. Defaults to 1 when `async_generate` is enabled.
    """
    # generation args
    top_k: int = 50
    top_p: float = 0.9
    ...
    # Async generate staleness control
