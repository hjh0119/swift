...
def dft_loss_func(outputs, labels, **kwargs):
    """
    ...
    sft_loss = F.cross_entropy(logits, labels, ignore_index=-100, reduction='none')

    # Compute detached probabilities for reweighting
    # This implements the sg(π_θ(y*_t | y*_<t, x)) part of the formula
    safe_labels = torch.where(labels == -100, 0, labels)
    with torch.no_grad():
        target_probs = torch.softmax(logits, dim=-1).gather(1, safe_labels.unsqueeze(-1)).squeeze(-1).detach()

    # Apply DFT reweighting: multiply loss by detached target probabilities
    # This implements the sg(π_θ(y*_t | y*_<t, x)) * log π_θ(y*_t | y*_<t, x) part
    dft_loss = sft_loss * target_probs
